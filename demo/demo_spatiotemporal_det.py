    ...
    for i in range(na):
        ...
        for j in range(nfpa):
            ...
            for ann in anno:
                ...
                #box make(img,xy,xy,color,linesize)
                cv2.rectangle(frame, st, ed, (0,0,255), 6)
                # cv2.rectangle(frame, st, ed, plate[0], 1)
                #label make(action)
                lable_list=[]
                for k, lb in enumerate(label):
                    ...
                    diag0 = (d0_x,d0_y)
                    diag1 = (d1_x,d1_y)
                    lable_list.append(diag0)
                    lable_list.append(diag1)
                    lable_list.append(color)
                    # print("label_xy1",diag0)
                    # print("label_xy2",diag1)
                    # cv2.rectangle(frame, diag0, diag1, plate[k + 1], -1)
                    # cv2.putText(frame, text, location, FONTFACE, FONTSCALE,
                    #             FONTCOLOR, THICKNESS, LINETYPE)
                if len(lable_list)==3:
                    cv2.rectangle(frame, lable_list[0], lable_list[1], lable_list[2], -1)
                    cv2.putText(frame, text, location, FONTFACE, FONTSCALE,
                                    FONTCOLOR, THICKNESS, LINETYPE)
                if len(lable_list)>3:
                    cv2.rectangle(frame, lable_list[0], lable_list[1], lable_list[2], -1)
                    cv2.putText(frame, text, location, FONTFACE, FONTSCALE,
                                    FONTCOLOR, THICKNESS, LINETYPE)
                    cv2.rectangle(frame, lable_list[3], lable_list[4], lable_list[5], -1)
                    cv2.putText(frame, text, location, FONTFACE, FONTSCALE,
                                    FONTCOLOR, THICKNESS, LINETYPE)
                lable_list=[]
...
def detection_inference(args, frame_paths):
    ...
    """
    #載入faster RCNN model py跟check point
    model = init_detector(args.det_config, args.det_checkpoint, args.device)
    print("===========class======================")
    print(model.CLASSES[0])
    print(model.CLASSES[1])
    print(model.CLASSES[2])
    print(model.CLASSES[3])
